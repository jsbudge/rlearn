...
def genPoints(nn, e, offsets, method='uniform'):
    np.random.seed(666)
    _gx = 0
    _gy = 0
    if nn == 1:
        ...
        if method == 'uniform':
            _gx = np.random.rand(nn) * e.shape[0] - e.shape[0] / 2 + offsets[0]
            _gy = np.random.rand(nn) * e.shape[1] - e.shape[1] / 2 + offsets[1]
            _gz, _gv = e(_gx, _gy)
            while np.any(_gv == 0):
                bad_len = sum(_gv == 0)
...
print('Loading radar...')
radar = Radar(fnme, env.scp, use_xml_flightpath=False, presum=presum)
...
param_gpu = cupy.array(np.array([np.pi / radar.el_bw, np.pi / radar.az_bw,
                                 ...
                                 radar.near_slant_range / c0, 2e9 * upsample, 1, radar.prf]), dtype=np.float64)
...
if n_samples > 1e6:
    ...
    gx, gy, gz, gv = genPoints(n_samples, env, scp_offsets, method=rand_method)
...
for ch in tqdm(np.arange(0, n_pulses, chunk_sz)):
    ...
    fptx_gpu = cupy.array(np.ascontiguousarray(radar.pos(tt), dtype=np.float64))
    fprx_gpu = cupy.array(np.ascontiguousarray(radar.pos(tt), dtype=np.float64))
    ...
    rpf_r_gpu = cupy.random.rand(*rpf_shape, dtype=np.float64) * noise_level
    rpf_i_gpu = cupy.random.rand(*rpf_shape, dtype=np.float64) * noise_level
    ...
    if do_backproject:
        ...
        backproject[blocks_per_grid_bpj, threads_per_block](fprx_gpu, bx_gpu, by_gpu, bz_gpu,
                                                            rb_gpu, rad_pan_gpu,

...
from simlib import getElevationMap, getElevation, enu2llh, llh2enu
from useful_lib import findPowerOf2
...
class SinglePulseBackground(Environment):
    ...
    def __init__(self):
        super().__init__()
        self.wd = WignerVilleDistribution

    def states(self):
        return dict(type='float', shape=(8,), min_value=0, max_value=1)
    ...
    def setParams(self, llr=None, bg_data=None, bg_res=None, ashfile=None, asifile=None,
        ...
        if bg_data is not None:
            ...
            bins = np.linspace(grid_data.min(), grid_data.max(), 1000)
    ...
    def __call__(self, x, y, is_enu=True, override_data=None):
        """
        ...
        if override_data is not None:
            ...
            dt = self._data
        d_int = dt[x1, y1] * xdiff * ydiff + dt[x1, y0] * xdiff * (1 - ydiff) + dt[x0, y1] * \
            (1 - xdiff) * ydiff + dt[x0, y0] * (1 - xdiff) * (1 - ydiff)

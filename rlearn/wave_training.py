...
motion_agent = Agent.create(agent='a2c', states=dict(currscan=dict(type='float', shape=(1,), min_value=env.az_lims[0], max_value=env.az_lims[1]),
...
for episode in tqdm(range(games)):
    ...
    print('Episode {}: return={} updates={}'.format(episode, sum_rewards, num_updates // 2))
...
for g in tqdm(range(eval_games)):
    ...
    while not terminal:
        ...
        wa, internals[0] = wave_agent.act(states=dict(cpi=states['cpi'],
                    currwave=states['currwave']), internals=internals[0],
...
for ant in range(env.n_tx):
    pulse = genPulse(np.linspace(0, 1, len(logs[log_num][5][:, 0])), logs[log_num][5][:, ant],
                     env.nr, env.nr / env.fs, env.fc, env.bw)
...
fig = plt.figure()
gs = gridspec.GridSpec(3, env.n_rx)
axes = []
for ant in range(env.n_rx):
...
for l in logs:
    ...
    dopp_freqs = np.fft.fftshift(np.fft.fftfreq(l[0].shape[1], (l[2][1] - l[2][0]))) / env.fc * c0

    # Draw the beam and platform
    bm_x, bm_y, bm_a, bm_b = env.env.getAntennaBeamLocation(l[2][0], l[3][0] + np.pi / 2, l[4][0])
    main_beam = ellipse(bm_x, bm_y, bm_a, bm_b, l[3][0])
    axes[env.n_rx].plot(main_beam[0, :], main_beam[1, :], 'gray')
    axes[env.n_rx].scatter(fpos[0], fpos[1], marker='*', c='blue')
    beam_dir = np.exp(-1j * l[3][0]) * fpos[2] / np.tan(l[4][0])
    axes[env.n_rx].arrow(fpos[0], fpos[1], beam_dir.imag, beam_dir.real)
    ...
    for idx, s in enumerate(env.env.targets):
        ...
        pos = np.array(pos)
        if len(pos) > 0:
            axes[env.n_rx].scatter(pos[:, 0], pos[:, 1], s=amp, c=pcols)
            ...
            plt_rng = 2 * (np.linalg.norm(env.env.pos(l[2])[:, -1] - np.array([pos[-1, 0], pos[-1, 1], 1])) -
                      ...
                      (l[2][-1] - l[2][0])
            axes[env.n_rx].text(pos[0, 0], pos[0, 1], f'{-plt_rng:.2f}', c='black')
    axes[env.n_rx].legend([f'{1 / (l[2][1] - l[2][0]):.2f}Hz: {l[2][-1]:.6f}'])
